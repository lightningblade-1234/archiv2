"""Script to generate crisis analytics and reports for shreeyan using local LLM."""
import sys
import os
import asyncio
...
def find_student_by_name(name: str, db):
    ...
    if not student:
        # Try by student_id
        student = db.query(Student).filter(
            Student.student_id.ilike(f"%{name}%")
        ).first()
    
    return student


async def generate_analytics_for_shreeyan():
    """Generate crisis analytics and reports for shreeyan using local LLM."""
    db = SessionLocal()
    ...
    try:
        ...
        # Initialize LLM client with LOCAL provider (not OpenAI)
        print("\n🤖 Initializing LOCAL LLM client...")
        llm_client = LLMClient(
            provider="local",
            base_url=settings.local_llm_base_url,
            model=settings.local_llm_model
        )
        print(f"   Using model: {settings.local_llm_model}")
        ...
        if alert_id:
            print(f"   Found existing alert: {alert_id}")
        else:
            print("   No existing alert found, creating analytics without alert_id")
        ...
        # Generate and save report using LOCAL LLM
        print("\n📝 Generating report using LOCAL LLM...")
        ...
        print("\n💡 These will now appear in the Admin Dashboard!")
